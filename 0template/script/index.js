...
(function () {
  ...
  /**
   * 引入神策
   * @param {String} env 环境 production/test
   */
  function initSa(env) {
    (function (para) {
      var p = para.sdk_url,
   ...
   * @param {Object} options 请求设置
   * url:请求URL
   * method: 请求方式，默认post
   * data: 请求参数
   * loading:是否自动加loading，默认false
   * success: 请求成功回调function
   * error: 请求失败回调function
   * complete: 请求完成回调function
   */
  var ajax = (function () {
    var xhr;
    ...
    return function (url, options) {
      ...
      if (options.method.toLowerCase() == 'get' && params && !url.match('?')) {
        url = url + '?' + params;
      }
  ...
  function wxjssdk(title, desc) {
    ...
    wx.ready(function () {
      wx.updateAppMessageShareData({
        title: window.wxShareData.title, // 分享标题
        desc: window.wxShareData.desc, // 分享描述
        link: window.wxShareData.link, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
        imgUrl: window.wxShareData.imgUrl, // 分享图标
      }, function () {
        //这里是回调函数 
        log('new share msg');
      });
